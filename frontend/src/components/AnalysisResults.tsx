...
export default function AnalysisResults({ result, xrayUrl, heatmapUrl, shapUrl, isLoading }: AnalysisResultsProps) {
  if (isLoading) {
    return (
          ...
          <div>
            ...
            <div className="space-y-5">
              {result.top_predictions.map((prediction) => (
                <div key={prediction.disease} className="space-y-2">
      ...
      <div className="mt-8 bg-surface-container-lowest p-6 rounded-3xl shadow-soft border border-outline-variant">
        <div className="flex justify-between items-start gap-4 flex-wrap mb-6">
          <div>
            ...
            <p className="text-xs text-secondary mt-1">Impact of clinical metadata attributes on model predictions</p>
          </div>
          <span className="bg-secondary-fixed text-on-secondary-fixed-variant px-3 py-1 rounded-full font-sans text-xs font-semibold">
        ...
        {shapUrl ? (
          <div className="relative rounded-2xl overflow-hidden border border-outline-variant shadow-sm flex items-center justify-center bg-white p-4">
            <img
              referrerPolicy="no-referrer"
